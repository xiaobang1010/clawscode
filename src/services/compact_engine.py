...
class PartialCompactDirection(str, Enum):
    FROM = "from"
    UP_TO = "up_to"
...
PARTIAL_COMPACT_UP_TO_PROMPT = """你的任务是对这份对话创建一份详细的摘要。这份摘要将放在继续会话的开头；后续会有新消息基于此上下文继续。请全面摘要，使只阅读摘要和新消息的人能完全理解发生了什么并继续工作。
...
7. **待办任务**：概述所有待办任务
8. **已完成工作**：描述到此为止完成了什么
9. **继续工作的上下文**：摘要任何理解并继续后续工作所需的上下文、决策或状态
